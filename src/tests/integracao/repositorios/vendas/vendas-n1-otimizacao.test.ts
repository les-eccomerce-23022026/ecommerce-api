...
describe('Integração - RepositorioVendasPostgres - Correção N+1 Queries', () => {
  ...
  afterEach(async () => {
    ...
    for (const cpf of cpfs) {
      await contexto.db!.executar(`DELETE FROM livraria_comercial.itens_venda WHERE ven_id IN (SELECT ven_id FROM livraria_comercial.vendas WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = $1))`, [cpf]);
      await contexto.db!.executar(`DELETE FROM livraria_comercial.vendas WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = $1)`, [cpf]);
      await contexto.db!.executar(`DELETE FROM livraria_gestao.clientes WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = $1)`, [cpf]);
      await contexto.db!.executar(`DELETE FROM livraria_gestao.usuarios WHERE usu_cpf = $1`, [cpf]);
    }
    await contexto.db!.executar(`DELETE FROM livraria_comercial.estoques WHERE liv_id IN (SELECT liv_id FROM livraria_comercial.livros WHERE liv_titulo LIKE 'Livro Teste%')`);
    await contexto.db!.executar(`DELETE FROM livraria_comercial.livros WHERE liv_titulo LIKE 'Livro Teste%' OR liv_titulo = 'Livro Status'`);
  });
  ...
  describe('listarPorUsuario - Sem N+1 Query', () => {
    it('deve listar vendas de um usuário com itens em uma única query', async () => {
      ...
      await contexto.db!.executar(`DELETE FROM livraria_gestao.clientes WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = '12345678901')`);
      ...
      const livroRes = await contexto.db!.executar<{ liv_uuid: string }>(
        `INSERT INTO livraria_comercial.livros (liv_uuid, liv_titulo, liv_isbn, aut_id, edi_id, gpr_id, liv_ano, liv_edicao, liv_numero_paginas, liv_altura, liv_largura, liv_peso, liv_profundidade, liv_codigo_barras, liv_ativo)
      ...
      // Setup: Criar estoque
      await contexto.db!.executar(
        `INSERT INTO livraria_comercial.estoques (liv_id, etq_quantidade_disponivel, etq_quantidade_reservada, etq_preco_venda, etq_valor_custo_atual, etq_ativo, loj_id)
         VALUES ((SELECT liv_id FROM livraria_comercial.livros WHERE liv_uuid = $1), 10, 0, 50.00, 30.00, TRUE, 1)`,
        [livroUuid]
      );
      ...
      const vendasPromises = Array.from({ length: 3 }, async () => {
        ...
        await contexto.db!.executar(
          `INSERT INTO livraria_comercial.itens_venda (ven_id, liv_uuid, itv_quantidade, itv_preco_unitario, loj_id)
           VALUES (
    ...
    it('deve listar venda com múltiplos itens (boundary test)', async () => {
      // Setup: Criar usuário
      // Setup: Limpar usuário existente para evitar duplicate key
      await contexto.db!.executar(`DELETE FROM livraria_gestao.clientes WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = '12345678902')`);
    ...
    it('deve listar vendas com status variados', async () => {
      // Setup: Criar usuário
      // Setup: Limpar usuário existente para evitar duplicate key
      await contexto.db!.executar(`DELETE FROM livraria_gestao.clientes WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = '12345678903')`);
        ...
        statuses.map(async (status) => {
          const vendaRes = await contexto.db!.executar<{ ven_uuid: string }>(
            `INSERT INTO livraria_comercial.vendas (usu_id, stv_id, ven_total_itens, ven_frete, ven_total_venda, loj_id)
             VALUES (
               (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_uuid = $1),
               (SELECT stv_id FROM livraria_comercial.status_venda WHERE stv_descricao = $2),
               50.00, 10.00, 60.00, 1
             )
             RETURNING ven_uuid`,
            [usuarioUuid, status]
    ...
    it('deve listar venda sem itens (LEFT JOIN edge case)', async () => {
      // Setup: Criar usuário
      // Setup: Limpar usuário existente para evitar duplicate key
      await contexto.db!.executar(`DELETE FROM livraria_gestao.clientes WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = '12345678904')`);
  ...
  describe('listarTodas - Sem N+1 Query', () => {
    it('deve listar todas as vendas com itens em uma única query', async () => {
      // Setup: Limpar usuário existente para evitar duplicate key
      await contexto.db!.executar(`DELETE FROM livraria_gestao.clientes WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = '11111111111')`);
      ...
      // Setup: Limpar usuário existente para evitar duplicate key
      await contexto.db!.executar(`DELETE FROM livraria_gestao.clientes WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = '22222222222')`);
      await contexto.db!.executar(`DELETE FROM livraria_comercial.vendas WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = '22222222222')`);
      await contexto.db!.executar(`DELETE FROM livraria_gestao.usuarios WHERE usu_cpf = '22222222222'`);
  ...
  describe('obterPorUuid - Sem Subquery Desnecessária', () => {
    it('deve obter venda com itens usando LEFT JOIN', async () => {
      // Setup
      // Setup: Limpar usuário existente para evitar duplicate key
      await contexto.db!.executar(`DELETE FROM livraria_gestao.clientes WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = '33333333333')`);
