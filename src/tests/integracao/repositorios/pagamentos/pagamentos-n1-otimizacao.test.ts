...
describe('Integração - RepositorioPagamentosPostgres - Correção N+1 Query', () => {
  ...
  afterEach(async () => {
    ...
    for (const cpf of cpfs) {
      await contexto.db!.executar(`DELETE FROM livraria_financeiro.cartao_pagamento WHERE pag_id IN (SELECT pag_id FROM livraria_financeiro.pagamento WHERE ven_id IN (SELECT ven_id FROM livraria_comercial.vendas WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = $1)))`, [cpf]);
      await contexto.db!.executar(`DELETE FROM livraria_financeiro.pagamento WHERE ven_id IN (SELECT ven_id FROM livraria_comercial.vendas WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = $1))`, [cpf]);
  ...
  describe('listarPorVenda - Sem N+1 Query', () => {
    it('deve listar pagamentos de uma venda com cartões em uma única query', async () => {
      ...
      await contexto.db!.executar(
        `INSERT INTO livraria_financeiro.bandeiras_cartao (ban_descricao) VALUES ('VISA') ON CONFLICT DO NOTHING`
      );
      ...
      const pagamentosPromises = Array.from({ length: 3 }, async () => {
        const pagamentoRes = await contexto.db!.executar<{ pag_uuid: string; pag_id: number }>(
          `INSERT INTO livraria_financeiro.pagamento (ven_id, tpg_id, stp_id, pag_valor, pag_criado_em, loj_id)
           VALUES (
             (SELECT ven_id FROM livraria_comercial.vendas WHERE ven_uuid = $1),
             (SELECT tpg_id FROM livraria_financeiro.tipo_pagamento WHERE tpg_descricao = 'cartao_credito'),
             (SELECT stp_id FROM livraria_financeiro.status_pagamento WHERE stp_descricao = 'APROVADO'),
             50.00, NOW(), 1
           )
        ...
        await contexto.db!.executar(
          `INSERT INTO livraria_financeiro.cartao_pagamento (pag_id, cpp_numero_tokenizado, cpp_nome_titular, cpp_validade, cpp_bandeira, loj_id)
    ...
    it('deve retornar array vazio quando venda não possui pagamentos', async () => {
      // Setup: Limpar usuário existente para evitar duplicate key
      await contexto.db!.executar(`DELETE FROM livraria_gestao.clientes WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = '55555555555')`);
    ...
    it('deve listar pagamentos com cupom de troca sem cartão', async () => {
      // Setup: Limpar usuário existente para evitar duplicate key
      await contexto.db!.executar(`DELETE FROM livraria_gestao.clientes WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = '44444444444')`);
      ...
      await contexto.db!.executar(
        `INSERT INTO livraria_financeiro.pagamento (ven_id, tpg_id, stp_id, pag_valor, pag_criado_em, pag_detalhes_cupom, loj_id)
         VALUES (
           (SELECT ven_id FROM livraria_comercial.vendas WHERE ven_uuid = $1),
           (SELECT tpg_id FROM livraria_financeiro.tipo_pagamento WHERE tpg_descricao = 'cupom_troca'),
           (SELECT stp_id FROM livraria_financeiro.status_pagamento WHERE stp_descricao = 'APROVADO'),
    ...
    it('deve listar pagamentos com status variados', async () => {
      // Setup: Limpar usuário existente para evitar duplicate key
      await contexto.db!.executar(`DELETE FROM livraria_gestao.clientes WHERE usu_id IN (SELECT usu_id FROM livraria_gestao.usuarios WHERE usu_cpf = '66666666666')`);
          ...
          contexto.db!.executar(
            `INSERT INTO livraria_financeiro.pagamento (ven_id, tpg_id, stp_id, pag_valor, pag_criado_em, loj_id)
             VALUES (
               (SELECT ven_id FROM livraria_comercial.vendas WHERE ven_uuid = $1),
               (SELECT tpg_id FROM livraria_financeiro.tipo_pagamento WHERE tpg_descricao = 'cartao_credito'),
               (SELECT stp_id FROM livraria_financeiro.status_pagamento WHERE stp_descricao = $2),
               50.00, NOW(), 1
             )`,
            [vendaUuid, status]
          )
