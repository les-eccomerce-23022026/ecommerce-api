...
describe('Integração — Administrador comum (listagens e restrições)', () => {
  ...
  describe('Autenticação e papel', () => {
    it('identifica mestre com eAdminMestre true no login', async () => {
      const res = await realizarLogin(contexto.app, 'admin@livraria.com.br', 'Admin@123');

      expect(res.status).toBe(200);
      expect(res.body.dados.user.eAdminMestre).toBe(true);
    });

    it('identifica administrador comum com eAdminMestre false', async () => {
      ...
      expect(me.body.dados.user.eAdminMestre).toBe(false);
      expect(me.body.dados.user.email).toBe(email);
    });

    it('expõe eAdminMestre em GET /api/auth/me para o mestre', async () => {
      ...
      expect(me.status).toBe(200);
      expect(me.body.dados.user.eAdminMestre).toBe(true);
    });
  });
