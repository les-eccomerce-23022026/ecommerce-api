...
describe('Integração - ServicoVendas - Regras de Negócio', () => {
  ...
  describe('registrarPedidoVenda', () => {
    ...
    it('deve permitir parcelamento acima de R$ 80,00', async () => {
      ...
      const usuarioRes = await contexto.db!.executar<{ usu_uuid: string }>(
        `INSERT INTO usuarios (usu_nome, usu_email, usu_cpf, usu_senha_hash, pap_id, usu_ativo)
         VALUES ('Cliente Parcelamento', 'cliente.parcelamento@teste.com', '12345678901', 'hash', 1, TRUE)
         RETURNING usu_uuid`
      );
    ...
    it('deve permitir pagamento split com valor mínimo R$ 10,00', async () => {
      ...
      const usuarioRes = await contexto.db!.executar<{ usu_uuid: string }>(
        `INSERT INTO usuarios (usu_nome, usu_email, usu_cpf, usu_senha_hash, pap_id, usu_ativo)
         VALUES ('Cliente Split', 'cliente.split@teste.com', '12345678902', 'hash', 1, TRUE)
         RETURNING usu_uuid`
      );
      const usuarioUuid = usuarioRes[0].usu_uuid;
  ...
  describe('visualizarDetalhesVenda', () => {
    ...
    it('deve lançar erro quando cliente tenta acessar venda de outro usuário', async () => {
      ...
      const usuarioRes = await contexto.db!.executar<{ usu_uuid: string }>(
        `INSERT INTO usuarios (usu_nome, usu_email, usu_cpf, usu_senha_hash, pap_id, usu_ativo)
         VALUES ('Dono da Venda', 'dono@teste.com', '12345678901', 'hash', 1, TRUE)
         RETURNING usu_uuid`
      );
  ...
  describe('solicitarTroca', () => {
    ...
    it('deve lançar erro quando venda não está entregue', async () => {
      ...
      const usuarioRes = await contexto.db!.executar<{ usu_uuid: string }>(
        `INSERT INTO usuarios (usu_nome, usu_email, usu_cpf, usu_senha_hash, pap_id, usu_ativo)
         VALUES ('Cliente', 'cliente@teste.com', '12345678901', 'hash', 1, TRUE)
         RETURNING usu_uuid`
      );
      const usuarioUuid = usuarioRes[0].usu_uuid;
    ...
    it('deve lançar erro RN0043: prazo de 7 dias expirado', async () => {
      ...
      const vendaRes = await contexto.db!.executar<{ ven_uuid: string }>(
        `INSERT INTO vendas (usu_id, stv_id, ven_total_itens, ven_frete, ven_total_venda, ven_data_hora_entrega)
         VALUES (
           (SELECT usu_id FROM usuarios WHERE usu_uuid = $1),
           (SELECT stv_id FROM status_vendas WHERE stv_descricao = 'ENTREGUE'),
           50.00, 10.00, 60.00, NOW() - INTERVAL '8 days'
         )
         RETURNING ven_uuid`,
        [usuarioUuid]
      );
