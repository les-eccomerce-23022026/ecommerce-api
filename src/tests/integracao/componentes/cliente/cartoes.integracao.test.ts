...
describe('Integração - Cartões de Crédito', () => {
  ...
  describe('[L7] Bandeiras de cartão permitidas e rejeitadas', () => {
    it('deve aceitar cartões com bandeiras permitidas (Visa, Mastercard)', async () => {
      // Teste com Visa (já tem uuidBandeiraVisa)
      const resVisa = await request(contexto.app)
        .post('/api/clientes/perfil/cartoes')
        .set('Authorization', `Bearer ${tokenCliente}`)
        .send({
          uuidBandeira: uuidBandeiraVisa,
      ...
      expect(resVisa.body.dados.ultimosDigitosCartao).toBe('4242');

      // Buscar UUID da Mastercard
      const resBandeiraMaster = await contexto.db!.executar<{ ban_uuid: string }>(
        "SELECT ban_uuid FROM bandeiras_cartao WHERE ban_descricao = 'Mastercard' LIMIT 1"
      );

      if (resBandeiraMaster.length > 0) {
        const uuidBandeiraMaster = resBandeiraMaster[0].ban_uuid;

        // Teste com Mastercard
        const resMaster = await request(contexto.app)
          .post('/api/clientes/perfil/cartoes')
          .set('Authorization', `Bearer ${tokenCliente}`)
          .send({
            uuidBandeira: uuidBandeiraMaster,
            token: 'tok_test_master',
        ...
        expect(resMaster.body.dados.ultimosDigitosCartao).toBe('5555');
      }
    });
