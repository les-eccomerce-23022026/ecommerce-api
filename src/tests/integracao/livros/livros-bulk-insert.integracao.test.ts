...
describe('Integração — Bulk Insert de Livros', () => {
  ...
  describe('POST /api/admin/autores/lote', () => {
    ...
    it('retorna 403 para não admin', async () => {
      ...
      const resRegistro = await request(app)
        ...
        .send({
          nome: 'Cliente Teste',
          email: `cliente-${uuidv4()}@teste.com`,
          senha: 'Senha123@',
          cpf: '12345678901',
        });

      const tokenCliente = resRegistro.body.token;
  ...
  describe('POST /api/admin/livros/lote', () => {
    it('cria múltiplos livros em lote com transação', async () => {
      ...
      expect(res.status).toBe(201);
      expect(res.body.livros).toBeDefined();
      expect(Array.isArray(res.body.livros)).toBe(true);
      expect(res.body.livros.length).toBe(2);
      expect(res.body.quantidade).toBe(2);

      // Verificar se os livros foram criados com estoque
      expect(res.body.livros[0].estoque).toBe(10);
      expect(res.body.livros[0].preco).toBe(49.9);
      expect(res.body.livros[1].estoque).toBe(15);
      expect(res.body.livros[1].preco).toBe(59.9);
    });
    ...
    it('falha se autor não existe', async () => {
      ...
      expect(res.body.erro).toContain('Autor "Autor Inexistente" não encontrado');
    });
  ...
  describe('POST /api/admin/livros (individual)', () => {
    it('cria livro individual com transação', async () => {
      ...
      expect(res.status).toBe(201);
      expect(res.body.uuid).toBe(livro.uuid);
      expect(res.body.titulo).toBe(livro.titulo);
      expect(res.body.estoque).toBe(20);
      expect(res.body.preco).toBe(69.9);
    ...
    it('falha se editora não existe', async () => {
      ...
      expect(res.status).toBe(500);
      expect(res.body.erro).toContain('Editora "Editora Inexistente" não encontrada');
    });
  });
    ...
    it('despacha pedido usando PATCH (alterado de PUT)', async () => {
      ...
      const resRegistro = await request(app)
        ...
        .send({
          ...
          email: `cliente-despacho-${uuidv4()}@teste.com`,
          senha: 'Senha123@',
          cpf: '98765432100',
        });
      ...
      await request(app)
        ...
        .set('Authorization', `Bearer ${tokenCliente}`)
        .send({
          livroUuid: 'a1b2c3d4-e5f6-7890-1234-56789abcdef0',
          quantidade: 1,
        });
