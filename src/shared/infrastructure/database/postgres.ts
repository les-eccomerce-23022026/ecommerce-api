 ...
 */
const CHAVES_POSTGRES_INDIVIDUAIS = [
  'POSTGRES_HOST',
  'POSTGRES_PORT',
  'POSTGRES_USER',
  'POSTGRES_PASSWORD',
...
function listarVariaveisPostgresIndividuaisFaltantes(): string[] {
  return CHAVES_POSTGRES_INDIVIDUAIS.filter((chave) => process.env[chave] == null);
}
...
function validarVariaveisPostgres(): void {
  ...
  const faltando = listarVariaveisPostgresIndividuaisFaltantes();
