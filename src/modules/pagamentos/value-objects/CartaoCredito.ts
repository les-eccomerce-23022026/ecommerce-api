...
export class CartaoCredito {
  ...
  constructor(
    ...
    this.validarValidade(validade);
    this.validarBandeira(bandeira);

    // Tokenização simples (em produção, usar serviço como PCI DSS compliant)
    this.numeroTokenizado = this.tokenizarNumero(numero);
  ...
  private validarNumero(numero: string): void {
    ...
    if (!this.validarLuhn(numeroLimpo)) {
  ...
  private validarNomeTitular(nome: string): void {
    if (!nome || nome.trim().length < 2) {
      throw new Error('Nome do titular inválido');
    }
  }

  private validarValidade(validade: string): void {
  ...
  private validarBandeira(bandeira: string): void {
  ...
  private tokenizarNumero(numero: string): string {
